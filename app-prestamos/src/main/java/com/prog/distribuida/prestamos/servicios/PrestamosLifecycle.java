package com.prog.distribuida.prestamos.servicios;
...
@ApplicationScoped
public class PrestamosLifecycle {
    @Inject
    ...
    public void init(@Observes StartupEvent event, Vertx vertx) {
        System.out.println("*************************PrestamosLifecycle.init()");
        ...
        try {
            ...
            var tags = List.of(
                    "traefik.enable=true",
                    "traefik.http.routers.authors.rule=PathPrefix(`/app-prestamos`)",
                    "traefik.http.middlewares.authors-stripprefix.stripPrefix.prefixes=/app-prestamos",
            ...
            client.registerService(serviceOptions)
                    .onSuccess(it -> {
                        System.out.println("Prestamos Registered service: " + serviceId);
                    })
                    .onFailure(err -> {
                        err.printStackTrace();
                    });
    ...
    public void stop(@Observes ShutdownEvent event, Vertx vertx) {
        System.out.println("*************************PrestamosLifecycle.stop()");
        ConsulClientOptions options = new ConsulClientOptions()
                .setHost(consulHost)
                .setPort(consulPort);
        ConsulClient client = ConsulClient.create(vertx, options);
        ...
        client.deregisterService(serviceId)
                .onSuccess(it -> {
                    System.out.println("Prestamos service deregistred: " + serviceId);
                })
                .onFailure(err -> {
                    err.printStackTrace();
                });
